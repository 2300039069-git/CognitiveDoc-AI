...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col selection:bg-amber-500 selection:text-black relative overflow-hidden font-sans">
      ...
      {/* Admin Sidebar */}
      <aside
        className={`fixed top-0 bottom-0 left-0 z-50 bg-slate-900/85 backdrop-blur-2xl border-r border-amber-500/20 transition-all duration-300 flex flex-col shadow-2xl ${
          mobileMenuOpen
      ...
      >
        {/* Brand Header */}
        <div className="h-16 border-b border-amber-500/20 flex items-center justify-between px-4 bg-amber-500/[0.04]">
          <Link
            to="/admin/dashboard"
            onClick={() => setMobileMenuOpen(false)}
          ...
          >
            ...
            {(!collapsed || mobileMenuOpen) && (
              <div>
                ...
                <span className="text-[9px] text-amber-400 font-mono tracking-widest uppercase">Master Admin</span>
              </div>
            )}
          </Link>
        ...
        <div className="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
          {adminNavItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon className={`w-4 h-4 min-w-[16px] transition-colors ${active ? 'text-amber-400' : 'text-slate-400 group-hover:text-amber-300'}`} />
        ...
        <div className="p-3 border-t border-amber-500/20 bg-slate-950/60 space-y-2">
          {(!collapsed || mobileMenuOpen) && (
            <Link
              to="/dashboard"
              onClick={() => setMobileMenuOpen(false)}
              className="flex items-center justify-center gap-2 w-full py-2 px-3 rounded-xl text-xs font-bold bg-cyan-500/10 border border-cyan-500/30 text-cyan-300 hover:bg-cyan-500/20 transition-all"
            >
            ...
            <div className="flex items-center gap-2.5 overflow-hidden">
              ...
              {(!collapsed || mobileMenuOpen) && (
                <div className="overflow-hidden text-left">
                  ...
                  <p className="text-[10px] text-amber-400 font-mono uppercase tracking-widest">Super Administrator</p>
                </div>
              )}
            </div>
            {(!collapsed || mobileMenuOpen) && (
      ...
      <div className={`flex-1 flex flex-col transition-all duration-300 ${collapsed ? 'md:ml-20' : 'md:ml-64'}`}>
        {/* Top Header */}
        <header className="sticky top-0 z-30 h-16 bg-slate-950/80 backdrop-blur-2xl border-b border-amber-500/20 flex items-center justify-between px-4 sm:px-6 lg:px-8 transition-colors">
          <div className="flex items-center gap-2 sm:gap-3 overflow-hidden">
            {/* Mobile Hamburger Button */}
            <button
              onClick={() => setMobileMenuOpen(true)}
              className="md:hidden p-2 rounded-xl border border-amber-500/30 bg-white/[0.04] text-amber-300 hover:bg-white/10 transition-colors"
              aria-label="Open navigation menu"
            >
              <Menu className="w-5 h-5" />
            </button>

            <span className="px-2.5 py-1 rounded-full text-[10px] sm:text-[11px] font-mono font-bold uppercase tracking-wider bg-amber-500/10 text-amber-400 border border-amber-500/30">
              Admin Zone
            </span>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/[0.04] border border-white/10 text-slate-300 text-xs font-mono">
              <Database className="w-3.5 h-3.5 text-cyan-400" />
              <span className="text-cyan-400 font-bold">MongoDB Atlas</span>

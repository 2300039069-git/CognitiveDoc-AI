...
export default function UserDashboard() {
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      {/* Welcome Banner */}
      <div className="glass-panel rounded-3xl p-6 sm:p-8 border border-white/10 bg-gradient-to-r from-slate-900 via-slate-900/90 to-brand-950/40 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
        
        {/* Ambient banner glow */}
        <div className="absolute right-0 top-0 w-80 h-80 bg-cyan-500/10 blur-[90px] pointer-events-none -z-10" />
        ...
        <div className="flex flex-wrap items-center gap-2.5 sm:gap-3 w-full sm:w-auto">
          ...
          <Link
            to="/upload"
            className="flex-1 sm:flex-initial text-center px-4 py-3 rounded-2xl text-xs sm:text-sm font-semibold bg-white/[0.05] hover:bg-white/[0.1] border border-white/10 text-slate-200 transition-all backdrop-blur-xl"
          >
            Custom Ingest
          </Link>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-white group-hover:text-cyan-300 transition-colors">Upload & Analyze</h3>
              <p className="text-xs text-slate-400">PDF, DOCX, TXT optical ingest</p>
            </div>
          </div>
        ...
        >
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-white group-hover:text-indigo-300 transition-colors">RAG Q&A Assistant</h3>
              <p className="text-xs text-slate-400">Ask questions with page citations</p>
            </div>
          </div>
        ...
        >
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h3 className="text-sm font-bold text-white group-hover:text-emerald-300 transition-colors">Export Vault</h3>
              <p className="text-xs text-slate-400">Download summaries & transcripts</p>
            </div>
          </div>
      ...
      <div className="glass-panel rounded-3xl p-6 border border-white/10 space-y-5 shadow-2xl">
        ...
        {documents.length === 0 ? (
          <div className="text-center py-12 text-slate-400 space-y-3">
            ...
            <Link
              to="/upload"
              className="btn-shimmer inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl text-xs font-bold bg-gradient-to-r from-cyan-500 to-brand-600 text-white shadow-lg shadow-cyan-500/20"
            >
                  ...
                  <tr key={doc.id} className="hover:bg-white/[0.03] transition-colors">
                    <td className="py-3.5 pr-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-xl bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center text-cyan-400 font-bold uppercase text-[10px] font-mono">
                          {doc.file_type}
                        </div>
                        <div>
                          <p className="font-semibold text-white truncate max-w-xs">{doc.original_name}</p>
                          <p className="text-[10px] text-slate-400 font-mono">{new Date(doc.uploaded_at).toLocaleDateString()}</p>
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="py-3.5">
                      <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-mono font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">
                    ...
                    <td className="py-3.5 text-right space-x-2">
                      <Link
                        ...
                        className="px-3 py-1.5 rounded-xl text-xs font-semibold bg-cyan-500/10 border border-cyan-500/30 text-cyan-300 hover:bg-cyan-500/20 transition-all"
                      >
                        Summary
                      </Link>
                      <Link
                        to={`/chat?docId=${doc.id}`}
                        className="px-3 py-1.5 rounded-xl text-xs font-semibold bg-white/[0.05] border border-white/10 text-slate-300 hover:bg-white/[0.1] hover:text-white transition-all"
                      >
                        Chat Q&A
                      </Link>
                    </td>

...
export default function AIProcessing() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 py-8">
      {/* Top Banner */}
      <div className="glass-panel rounded-3xl p-8 border-slate-800 text-center space-y-4">
        <div className="w-16 h-16 mx-auto rounded-2xl bg-gradient-to-tr from-brand-600 to-indigo-600 p-0.5 shadow-xl shadow-brand-500/25 flex items-center justify-center">
          {completed ? (
        ...
        <div className="space-y-1">
          ...
          <p className="text-xs sm:text-sm text-slate-400">
            Document: <strong className="text-brand-400 font-mono">{docDetails?.original_name || 'Uploaded File'}</strong>
          </p>
        </div>

        {/* Progress Bar */}
        <div className="w-full max-w-md mx-auto bg-slate-950 rounded-full h-3 p-0.5 border border-slate-800 overflow-hidden">
          <div
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3.5">
                  <div className="w-8 h-8 rounded-xl bg-slate-900 border border-slate-800 flex items-center justify-center text-xs font-mono font-bold">
                    {isDone ? (
                  ...
                  <div>
                    <p className={`text-sm font-bold ${isDone || isCurrent ? 'text-white' : 'text-slate-400'}`}>
                      {step.title}
                    </p>
      ...
      {completed && (
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-2">
          <Link
            ...
            className="w-full sm:w-auto px-8 py-3.5 rounded-xl text-sm font-bold bg-brand-600 text-white hover:bg-brand-500 transition-all flex items-center justify-center gap-2 shadow-lg shadow-brand-600/30"
          >
          ...
          <Link
            to={`/chat?docId=${docId}`}
            className="w-full sm:w-auto px-8 py-3.5 rounded-xl text-sm font-semibold bg-slate-800 text-slate-200 hover:bg-slate-700 transition-all flex items-center justify-center gap-2"
          >

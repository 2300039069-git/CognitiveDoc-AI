  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (err.code !== 'auth/popup-closed-by-user' && err.code !== 'auth/cancelled-popup-request') {
        setError(err.message || 'Google registration encountered an issue. Please try email registration.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleDirectRegister = async (e) => {
    ...
    try {
      ...
      if (err.response?.data?.detail) {
        ...
        setError('Registration failed. Please check your credentials or sign in.');
      }
    } finally {
      setLoading(false);
    }
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3.5 pt-2">
                <div className="flex items-center gap-3 text-xs text-slate-700 dark:text-slate-300">
                  ...
                  <span>Direct Instant Registration (No OTP Required)</span>
                </div>
                <div className="flex items-center gap-3 text-xs text-slate-700 dark:text-slate-300">
                  ...
                  <span>Multi-Tenant MongoDB Atlas Cloud Storage</span>
                </div>
                <div className="flex items-center gap-3 text-xs text-slate-700 dark:text-slate-300">
                  ...
                  <span>11+ Multilingual Indic & Global Languages</span>
                </div>
              </div>
            </div>
          ...
          <div className="lg:col-span-7 space-y-6">
            <div className="text-center space-y-2 lg:text-left">
              ...
              <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400">
                Enter your details to instantly create your account and access your workspace
              </p>
            </div>
            ...
            <div className="glass-panel rounded-3xl p-6 sm:p-8 space-y-5 shadow-2xl">
              ...
              <button
                ...
                disabled={loading}
                className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-2xl border border-slate-200 dark:border-white/10 hover:border-cyan-500/40 bg-white hover:bg-slate-50 dark:bg-white/[0.04] dark:hover:bg-white/[0.08] text-slate-800 dark:text-white text-xs font-bold transition-all shadow-sm cursor-pointer"
              >
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="e.g. Acme Corp / Enterprise Team"
                      className="w-full pl-10 pr-4 py-2.5 rounded-2xl bg-white dark:bg-slate-950/80 border border-slate-200 dark:border-white/10 focus:border-brand-500 dark:focus:border-cyan-500 focus:ring-1 focus:ring-brand-500 dark:focus:ring-cyan-500 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 text-xs sm:text-sm outline-none transition-all shadow-sm"
                    />
                  </div>
                </div>
